...
namespace ImPr
{
    class BackgroundHelper
    {
        ...
        public async static Task<bool> SendClick(IntPtr hwnd, int x, int y, int Delay)
        {
            ...
            bool result;
            result = PostMessage(hwnd, (uint)MouseEvents.WM_MOVE, 0, MakeLParam(x, y));
            await Task.Delay(Delay / 4);
            result = PostMessage(hwnd, (uint)MouseEvents.WM_KEYDOWN, 1, MakeLParam(x, y));
            await Task.Delay(Delay / 4);
            result = PostMessage(hwnd, (uint)MouseEvents.WM_KEYCLK, 1, MakeLParam(x, y));
            await Task.Delay(Delay / 4);
            result = PostMessage(hwnd, (uint)MouseEvents.WM_KEYUP, 0, MakeLParam(x, y));
            await Task.Delay(5);
            result = PostMessage(hwnd, (uint)MouseEvents.WM_MOVE, 0, MakeLParam(0, 0));
